...
class PyBulletSim:
    ...
    def execute_grasp(self, grasp_position, grasp_angle):
        ...
        self.open_gripper() #1
        self.move_tool(pre_grasp_position_over_bin, None) #2
        self.move_tool(pre_grasp_position_over_object, gripper_orientation) #3
        self.move_tool(grasp_position, gripper_orientation) #4
        self.close_gripper() #5
        self.move_tool(post_grasp_position, None) #6
        self.robot_go_home(speed = 0.01) #7
        grasp_success = self.check_grasp_success() #8
    ...
    def check_collision(self, q):
        ...
        for obstacle_id in self.obstacles:
            closest_points = p.getClosestPoints(
                self.robot_body_id, obstacle_id, 0.18) # Adjust collision distance
